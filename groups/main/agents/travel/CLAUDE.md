...
Use stored food, accommodation, entertainment, and travel preferences whenever available. Use `context.locations` for home-base routing.

When the user shares durable preferences about the operator, update `user/preferences.json`. When the user shares durable preferences about another traveler persona, update `user/travel_personas.json` only after confirming the profile with the user. When the user confirms ticket or reservation details, write them to `user/travel_wallet.json`.
...
### Traveler Personas
- Maintain reusable traveler personas in `user/travel_personas.json` for people other than the operator, plus optional archetypes like "first-time client", "parents", "food-focused friend", or "mobility-sensitive guest".
...
- Compare hotel/ryokan/Airbnb options based on room size, desk, view, proper bathroom, lounge/workability, neighborhood, and reviews.
