...
async function runQuery(
  ...
  if (claudeCodeExecutable) {
    ...
    log(
      'Claude Code executable resolver found no candidate; using SDK default',
    );
  }
...
async function main(): Promise<void> {
  ...
  if (isSessionSlashCommand) {
    ...
    if (trimmedPrompt === '/ops-health') {
      writeOutput({
        status: 'success',
        result:
          '/ops-health is generated by the NanoClaw host process. Run it from a live chat so it can report service, deploy, queue, IPC, and channel health without starting an agent session.',
        newSessionId: sessionId,
      });
      return;
    }
